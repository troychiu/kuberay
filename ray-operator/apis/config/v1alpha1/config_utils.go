...
import (
	...
	"github.com/ray-project/kuberay/ray-operator/controllers/ray/batchscheduler/native"
...
func ValidateBatchSchedulerConfig(logger logr.Logger, config Configuration) error {
	...
	if len(config.BatchScheduler) > 0 {
		...
		if config.BatchScheduler == volcano.GetPluginName() || config.BatchScheduler == yunikorn.GetPluginName() || config.BatchScheduler == schedulerplugins.GetPluginName() || config.BatchScheduler == kaischeduler.GetPluginName() || config.BatchScheduler == native.GetPluginName() {
